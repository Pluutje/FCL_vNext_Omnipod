...
class OmnipodDashPumpPlugin @Inject constructor(
    ...
    private fun deliverBasalCorrection(): PumpEnactResult {
        if (!podStateManager.needsBasalCorrection()) {
            aapsLogger.info(LTag.PUMP, "Basal correction no longer needed")
            return pumpEnactResultProvider.get().success(true).enacted(false).comment("Basal correction no longer needed")
        }
